...
Период дат по умолчанию для разных типов отелей, например для отелей 2 дня, а для санаториев 14 дней, тип отеля зависит от поля  в отеле `display_as`. Устанавливается если пользователь не посещал виджет, иначе установятся значения из прошлого посещения.
